...
matching `bg3se-macos` camera build and enables the tuned third-person camera
whenever the client camera becomes available.

Edit `CameraConfig.lua` to change pitch, two-state zoom distances, FOV, framing,
follow speed, or floor protection. These overrides are exploration-only:
combat uses BG3's vanilla tactical camera and leaving combat automatically
resumes the configured third-person profile. An in-game settings UI is still
pending. Controller-specific camera input is not part of the macOS target.
...
available. Combat automatically restores vanilla keyboard behavior and leaving
combat resumes direct movement. Install its directory beside the camera mod,
then add these bindings to `PlayerProfiles/Public/inputconfig_p1.json`:
